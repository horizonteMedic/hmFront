...
export default async function PCRULTRASENSIBLE(datos = {}, docExistente = null) {
  ...
  doc.text("BIOQUIMICA", pageW / 2, 38, { align: "center" });
  ...
  doc.text("SUERO EN AYUNAS", config.margin + 20, y);
  y += config.lineHeight;
  ...
  doc.text("PCR ULTRASENSIBLE (MÉTODO INMUNOTURBIDIMÉTRICO)" || '', config.col1X, y + 6,);

  // Resultado
  doc.setFont(config.font, "normal").setFontSize(config.fontSize.body);
  doc.text(String(datos.resultado) || '', config.col2X, y + 6, { align: "center" });

  // Valores
  doc.setFont(config.font, "normal").setFontSize(config.fontSize.body);
  doc.text("< 7.5 mg/l" || '', config.col3X, y + 6, { align: "left" });
