...
// Función para dibujar fondo de ojo
const drawFondoDeOjo = (doc, datos = {}, yInicio) => {
  // Parámetros de tabla
  const tablaInicioX = 5;
  const tablaAncho = 200;
  const filaAltura = 5;
  ...
  // Posiciones X bien calculadas y alineadas (total: 25 + 8 + 8 + 25 + 8 + 8 = 82mm, resto 108mm para hallazgos)
  const xInicio = tablaInicioX;
  const xAnormalLabel = xInicio;
  const xGrupo1OD = xAnormalLabel + anchoLabel;
  const xGrupo1OI = xGrupo1OD + anchoOD;
  const xNormalLabel = xGrupo1OI + anchoOI;
  const xGrupo2OD = xNormalLabel + anchoLabel;
  const xGrupo2OI = xGrupo2OD + anchoOD;
  const xHallazgoInicio = xGrupo2OI + anchoOI;
  const xHallazgoFin = tablaInicioX + tablaAncho;

  // Dibujar fila gris con todos los headers en la misma línea
  doc.setFillColor(196, 196, 196);
  doc.rect(tablaInicioX, yPos, tablaAncho, filaAltura, 'F');

  // Dibujar todas las líneas verticales
  doc.line(tablaInicioX, yPos, tablaInicioX, yPos + filaAltura);
  doc.line(xGrupo1OD, yPos, xGrupo1OD, yPos + filaAltura);
  doc.line(xGrupo1OI, yPos, xGrupo1OI, yPos + filaAltura);
  doc.line(xNormalLabel, yPos, xNormalLabel, yPos + filaAltura);
  doc.line(xGrupo2OD, yPos, xGrupo2OD, yPos + filaAltura);
  doc.line(xGrupo2OI, yPos, xGrupo2OI, yPos + filaAltura);
  doc.line(xHallazgoInicio, yPos, xHallazgoInicio, yPos + filaAltura);
  doc.line(tablaInicioX + tablaAncho, yPos, tablaInicioX + tablaAncho, yPos + filaAltura);
  ...
  const yCentro = yPos + filaAltura / 2 + 1;
  doc.setFont("helvetica", "bold").setFontSize(8);
  doc.text("FONDO DE OJO", xInicio + anchoLabel / 2, yCentro, { align: "center" });
  doc.setFont("helvetica", "bold").setFontSize(7);
  doc.text("OD", xGrupo1OD + anchoOD / 2, yCentro, { align: "center" });
  doc.text("OI", xGrupo1OI + anchoOI / 2, yCentro, { align: "center" });
  doc.text("OD", xGrupo2OD + anchoOD / 2, yCentro, { align: "center" });
  doc.text("OI", xGrupo2OI + anchoOI / 2, yCentro, { align: "center" });
  doc.text("HALLAZGO", xHallazgoInicio + (xHallazgoFin - xHallazgoInicio) / 2, yCentro, { align: "center" });
  yPos += filaAltura;

  // Una sola fila con: anormal | OD | OI | normal | OD | OI | HALLAZGO
  // Calcular altura dinámica para hallazgos si hay mucho texto
  const hallazgosFondoOjo = String(datos.txtFoHallazgos || "");
  const anchoHallazgo = xHallazgoFin - xHallazgoInicio - 4;

  // Calcular altura necesaria para hallazgos
  ...
  const alturaFilaHallazgos = calcularAlturaHallazgos(hallazgosFondoOjo, anchoHallazgo);
  const yCentroFila = yPos + alturaFilaHallazgos / 2 + 1;

  // Dibujar todas las líneas verticales
  doc.line(tablaInicioX, yPos, tablaInicioX, yPos + alturaFilaHallazgos);
  doc.line(xGrupo1OD, yPos, xGrupo1OD, yPos + alturaFilaHallazgos);
  doc.line(xGrupo1OI, yPos, xGrupo1OI, yPos + alturaFilaHallazgos);
  doc.line(xNormalLabel, yPos, xNormalLabel, yPos + alturaFilaHallazgos);
  doc.line(xGrupo2OD, yPos, xGrupo2OD, yPos + alturaFilaHallazgos);
  doc.line(xGrupo2OI, yPos, xGrupo2OI, yPos + alturaFilaHallazgos);
  doc.line(xHallazgoInicio, yPos, xHallazgoInicio, yPos + alturaFilaHallazgos);
  ...
  // Label "anormal" - mismo tipo y tamaño que "normal"
  doc.setFont("helvetica", "normal").setFontSize(7);
  doc.text("anormal", xAnormalLabel + 1, yCentroFila);

  // Marcar X si está seleccionado para anormal (primer grupo OD/OI)
  if (datos.rbfOAnormalOd === true) {
    dibujarX(doc, xGrupo1OD + anchoOD / 2, yCentroFila);
  }
  if (datos.rbfOAnormalOi === true) {
    dibujarX(doc, xGrupo1OI + anchoOI / 2, yCentroFila);
  }

  // Label "normal" - mismo tipo y tamaño que "anormal"
  doc.setFont("helvetica", "normal").setFontSize(7);
  doc.text("normal", xNormalLabel + 1, yCentroFila);

  // Marcar X si está seleccionado para normal (segundo grupo OD/OI)
  if (datos.rbfONormalOd === true) {
    dibujarX(doc, xGrupo2OD + anchoOD / 2, yCentroFila);
  }
  if (datos.rbfONormalOi === true) {
    dibujarX(doc, xGrupo2OI + anchoOI / 2, yCentroFila);
  }

  // Texto de hallazgos fondo de ojo - puede crecer dinámicamente
  doc.setFont("helvetica", "normal").setFontSize(7);
  dibujarTextoConSaltoLinea(doc, hallazgosFondoOjo, xHallazgoInicio + 2, yPos + 2, anchoHallazgo);

  yPos += alturaFilaHallazgos;

  // Sección: PIO - Header gris con PIO: | OD | OI | NO APLICA en la misma línea
  doc.setFillColor(196, 196, 196);
  doc.rect(tablaInicioX, yPos, tablaAncho, filaAltura, 'F');
  ...
  const xPIOOI = xPIOOD + anchoPIO;

  // Dibujar líneas verticales
  doc.line(tablaInicioX, yPos, tablaInicioX, yPos + filaAltura);
  doc.line(xPIOLabelEnd, yPos, xPIOLabelEnd, yPos + filaAltura);
  doc.line(xPIOOD, yPos, xPIOOD, yPos + filaAltura);
  doc.line(xPIOOI, yPos, xPIOOI, yPos + filaAltura);
  doc.line(tablaInicioX + tablaAncho, yPos, tablaInicioX + tablaAncho, yPos + filaAltura);
  ...
  const yCentroPIO = yPos + filaAltura / 2 + 1;
  doc.setFont("helvetica", "bold").setFontSize(8);
  doc.text("PIO:", tablaInicioX + 2, yCentroPIO);
  doc.setFont("helvetica", "bold").setFontSize(7);
  doc.text("OD", xPIOLabelEnd + anchoPIO / 2, yCentroPIO, { align: "center" });
  doc.text("OI", xPIOOD + anchoPIO / 2, yCentroPIO, { align: "center" });
  doc.text("NO APLICA", xPIOOI + anchoPIO / 2, yCentroPIO, { align: "center" });
  yPos += filaAltura;

  // Fila de valores PIO
  const yCentroPIOValores = yPos + filaAltura / 2 + 1;
  doc.line(tablaInicioX, yPos, tablaInicioX, yPos + filaAltura);
  doc.line(xPIOLabelEnd, yPos, xPIOLabelEnd, yPos + filaAltura);
  doc.line(xPIOOD, yPos, xPIOOD, yPos + filaAltura);
  doc.line(xPIOOI, yPos, xPIOOI, yPos + filaAltura);
  ...
  doc.setFont("helvetica", "normal").setFontSize(7);
  const pioOD = String(datos.txtPioOd || "");
  const pioOI = String(datos.txtPioOi || "");
  const pioNA = String(datos.txtPioNa || "");
  doc.text(pioOD, xPIOLabelEnd + anchoPIO / 2, yCentroPIOValores, { align: "center" });
  doc.text(pioOI, xPIOOD + anchoPIO / 2, yCentroPIOValores, { align: "center" });
  doc.text(pioNA, xPIOOI + anchoPIO / 2, yCentroPIOValores, { align: "center" });
  yPos += filaAltura;
...
const drawAgudezaVisual = (doc, datos = {}, yInicio) => {
  ...
  // Estructura: [Binocular (Reev.)] | [Sin Correctores] | [Con Correctores] | [Con Agujero Estenopeico]
  doc.rect(tablaInicioX, yPos, tablaAncho, filaAltura);
  doc.line(xLabelEnd, yPos, xLabelEnd, yPos + filaAltura);
  doc.line(x2, yPos, x2, yPos + filaAltura);
  doc.line(x4, yPos, x4, yPos + filaAltura);
  ...
  const binSC = getValor(
    // data real
    "txtBinocularSinCorregir",
    // alternativos
    "txtAvBinocularSinCorrectores",
    "avBinocularSinCorrectores",
    "binocularSinCorrectores",
    "avBinSc"
  );
  const binCC = getValor(
    // data real
    "txtBinocularCorregida",
    // alternativos
    "txtAvBinocularConCorrectores",
    "avBinocularConCorrectores",
    "binocularConCorrectores",
    "avBinCc"
  );
  const binAE = getValor(
    "txtAvBinocularAgujeroEstenopeico",
    "avBinocularAgujeroEstenopeico",
    "binocularAgujeroEstenopeico",
    "avBinAe"
  );

  // Centrar dentro de cada celda combinada (2 columnas)
  doc.setFont("helvetica", "bold").setFontSize(7);
...
// Función para dibujar Test de Evaluación Complementaria (filas modernas)
const drawTestEvaluacionComplementaria = (doc, datos = {}, yInicio) => {
  ...
  // Layout por opción: [X] [Texto] (x3 opciones)
  const wX = 8;
  const wTxt = 22;
  ...
  const drawFila = ({
    ...
    // separadores internos por celdas: X/texto
  ...
  // Fila 2: Colores Puros
  drawFila({
    label: "Test de Colores Puros (Rojo-Amarillo-Verde)",
    // nota: en data viene como rbtEcColeres*
  ...
  // Fila 3: Estereopsia (Test profundidad) + segundos
...
// Función para dibujar Refracción + tablas (Lejos/Cerca) + Agudeza Visual Final (con refracción)
const drawRefraccion = (doc, datos = {}, yInicio) => {
  ...
  // Tablas lado a lado: DE LEJOS / DE CERCA
  const gap = 0; // sin vacío entre tablas
  ...
  const wRowLabel = 12;
  const wData = (tableW - wRowLabel) / 4; // SF, CIL, EJE, DIP
  ...
  const drawTablaRefraccion = (x, titulo, prefKey) => {
    ...
    const drawDataRow = (label, ojoKey) => {
      ...
      doc.setFont("helvetica", "normal").setFontSize(7);
      // data real: txtLejosOdSf / txtCercaOiEje / etc.
      const prefTxt = prefKey === "refraccionLejos" ? "txtLejos" : prefKey === "refraccionCerca" ? "txtCerca" : null;
      const txtOjo = ojoKey; // "Od" | "Oi"
      const sf = getValor(
        prefTxt ? `${prefTxt}${txtOjo}Sf` : null,
        `${prefKey}${ojoKey}Sf`,
        `${prefKey}${ojoKey}SF`,
        `${prefKey}${ojoKey}_sf`
      );
      const cil = getValor(
        prefTxt ? `${prefTxt}${txtOjo}Cil` : null,
        `${prefKey}${ojoKey}Cil`,
        `${prefKey}${ojoKey}CIL`,
        `${prefKey}${ojoKey}_cil`
      );
      const eje = getValor(
        prefTxt ? `${prefTxt}${txtOjo}Eje` : null,
        `${prefKey}${ojoKey}Eje`,
        `${prefKey}${ojoKey}EJE`,
        `${prefKey}${ojoKey}_eje`
      );
      const dip = getValor(
        prefTxt ? `${prefTxt}${txtOjo}Dip` : null,
        `${prefKey}${ojoKey}Dip`,
        `${prefKey}${ojoKey}DIP`,
        `${prefKey}${ojoKey}_dip`
      );
  ...
  // Header: AGUDEZA VISUAL FINAL (CON REFRACCIÓN)
  doc.setFillColor(196, 196, 196);
  doc.rect(tablaInicioX, yPos, tablaAncho, filaAltura, "F");
  doc.rect(tablaInicioX, yPos, tablaAncho, filaAltura);
  doc.setFont("helvetica", "bold").setFontSize(8);
  ...
  drawMiniAV(
    xLejos,
    "De Lejos",
    ["txtAvConRefraccionLejosOd", "avFinalLejosOd", "txtAvFinalLejosOd", "agudezaVisualFinalLejosOd"],
    ["txtAvConRefraccionLejosOi", "avFinalLejosOi", "txtAvFinalLejosOi", "agudezaVisualFinalLejosOi"]
  );
  drawMiniAV(
    xCerca,
    "De Cerca",
    ["txtAvConRefraccionCercaOd", "avFinalCercaOd", "txtAvFinalCercaOd", "agudezaVisualFinalCercaOd"],
    ["txtAvConRefraccionCercaOi", "avFinalCercaOi", "txtAvFinalCercaOi", "agudezaVisualFinalCercaOi"]
  );
...
// Función para dibujar Indicadores y Restricciones (compacto en columnas)
const drawIndicadoresRestricciones = (doc, datos = {}, yInicio) => {
  ...
  // En tablas INDICADORES/RESTRICCIONES la celda de check es solo para la "X"
  ...
  // Columnas: [label1][( )][label2][( )][Cerca][( )][Lejos][( )]
  const wL1 = 55;
  const wC1 = 10;
  const wL2 = 55;
  const wC2 = 10;
  ...
  const x6 = x5 + wCCerca;
  const x7 = x6 + wLLejos;
  // const x8 = x7 + wCLejos; // borde final
  ...
  // Fila adicional INDICADORES: 3 pares (label | ( ))
  const drawIndicadoresRow3 = (cells) => {
    ...
    drawXCell(x + w - wC, wC, yMid, cells.check3);
  ...
  const drawRestrRow2 = (a, b) => {
    ...
    drawXCell(x + w - wCheck, wCheck, yMid, b.check);
    y += filaAltura;
  };
  ...
  const drawRestrRow3 = (a, b, c, boldA = false) => {
    ...
    drawXCell(x + w - wCheck, wCheck, yMid, c.check);
    y += filaAltura;
  };

  drawRestrRow1(
    "No restringe actividades laborales en el puesto de trabajo",
    getBool("chkR1", "resNoRestringe", "restriccionesNoRestringe", "noRestringe")
  );

  drawRestrRow2(
    {
      label: "No trabajos con cables eléctricos, ni fibra óptica",
      check: getBool("chkR3", "resNoCables", "restriccionesNoCablesFibra", "noCablesFibra"),
    },
    {
      label: "No conducción Vehicular",
      check: getBool("chkR4", "resNoConduccion", "restriccionesNoConduccion", "noConduccionVehicular"),
    }
  );

  drawRestrRow3(
    {
      label: "Uso de Correctores Oculares",
      check: getBool("chkR2", "chkR2Lejos", "chkR2Cerca", "resUsoCorrectores", "restriccionesUsoCorrectores", "usoCorrectoresRestriccion"),
    },
    {
      label: "Lejos",
      check: getBool("chkR2Lejos", "resLejos", "restriccionesLejos", "usoLejosRestriccion"),
    },
    {
      label: "Cerca (Trabajos de Oficina)",
      check: getBool("chkR2Cerca", "resCerca", "resCercaOficina", "restriccionesCerca", "usoCercaRestriccion"),
    },
    true
  );
...
export default async function EvaluacionOftalmologica2021_Digitalizado(datos = {}, docExistente = null) {
  ...
  const yDespuesExamen = drawExamenClinicoExterno(doc, datos, yDespuesDatos);

  // 5) Fondo de ojo
  const yDespuesFondoOjo = drawFondoDeOjo(doc, datos, yDespuesExamen);

  // 6) Agudeza visual
  const yDespuesAgudeza = drawAgudezaVisual(doc, datos, yDespuesFondoOjo);

  // 7) Test de evaluación complementaria
