...
export default function B_FichaDetencionSAS2({ data = {} }) {
  useEffect(() => {
    ...
    const datosReales = {
      ...
      // Datos de checkboxes para SAS
      apneaSueno: {
        si: Boolean(data.apneaSuenoSi ?? false),
        no: Boolean(data.apneaSuenoNo ?? false),
        aplica: Boolean(data.apneaSuenoAplica ?? false)
      },
      hta: {
        si: Boolean(data.htaSi ?? false),
        no: Boolean(data.htaNo ?? false),
        aplica: Boolean(data.htaAplica ?? false)
      },
      polisomnografia: {
        si: Boolean(data.polisomnografiaSi ?? false),
        no: Boolean(data.polisomnografiaNo ?? false),
        aplica: Boolean(data.polisomnografiaAplica ?? false)
      },
      choqueVehiculo: {
        enMina: {
          si: Boolean(data.choqueVehiculoEnMinaSi ?? false),
          no: Boolean(data.choqueVehiculoEnMinaNo ?? false)
        },
        fueraMina: {
          si: Boolean(data.choqueVehiculoFueraMinaSi ?? false),
          no: Boolean(data.choqueVehiculoFueraMinaNo ?? false)
        },
        aplica: Boolean(data.choqueVehiculoAplica ?? false)
      },
      // Detalle de choques
      detalleChoques: String(data.detalleChoques ?? ""),
      // Criterio 1
      criterio1: {
        cabeceoAccidente: Boolean(data.criterio1CabeceoAccidente ?? false)
      },
      // Criterio 2 (2 o más es positivo)
      criterio2: {
        accidenteUltimas5Horas: Boolean(data.criterio2AccidenteUltimas5Horas ?? false),
        ausenciaManiobraEvasiva: Boolean(data.criterio2AusenciaManiobraEvasiva ?? false),
        colisionFrontal: Boolean(data.criterio2ColisionFrontal ?? false),
        vehiculoInvadioCarril: Boolean(data.criterio2VehiculoInvadioCarril ?? false),
        conductorNoRecuerda: Boolean(data.criterio2ConductorNoRecuerda ?? false),
        conductorTomoMedicacion: Boolean(data.criterio2ConductorTomoMedicacion ?? false),
        conductorHorasExtra: Boolean(data.criterio2ConductorHorasExtra ?? false)
      },
      // Clasificación de choques
      clasificacionChoques: {
        accidenteConfirmadoSomnolencia: Boolean(data.clasificacionAccidenteConfirmadoSomnolencia ?? false),
        accidenteAltaSospechaSomnolencia: Boolean(data.clasificacionAccidenteAltaSospechaSomnolencia ?? false),
        accidenteEscasaEvidenciaSomnolencia: Boolean(data.clasificacionAccidenteEscasaEvidenciaSomnolencia ?? false),
        noDatosSuficientes: Boolean(data.clasificacionNoDatosSuficientes ?? false),
        accidenteNoDebidoSomnolencia: Boolean(data.clasificacionAccidenteNoDebidoSomnolencia ?? false)
      },
      // Entrevista al paciente
      entrevistaPaciente: {
        parejaComentoRonca: Boolean(data.entrevistaParejaComentoRonca ?? false),
        parejaComentoRuidosRespirar: Boolean(data.entrevistaParejaComentoRuidosRespirar ?? false),
        parejaComentoPausaRespiratoria: Boolean(data.entrevistaParejaComentoPausaRespiratoria ?? false),
        masSueñoQueCompañeros: Boolean(data.entrevistaMasSueñoQueCompañeros ?? false)
      },
      // Escala de Epworth
      escalaEpworth: {
        puntuacionTotal: Number(data.escalaEpworthPuntuacionTotal ?? 0)
      },
      // Datos de examen físico para página 2
      pesoPaciente: String(data.pesoPaciente ?? ""),
      tallaPaciente: String(data.tallaPaciente ?? ""),
      imcPaciente: String(data.imcPaciente ?? ""),
      circunferenciaCuello: String(data.circunferenciaCuello ?? ""),
      circunferenciaVaronNormal: Boolean(data.circunferenciaVaronNormal ?? false),
      circunferenciaMujerNormal: Boolean(data.circunferenciaMujerNormal ?? false),
      presionSistolica: String(data.presionSistolica ?? ""),
      presionDiastolica: String(data.presionDiastolica ?? ""),
      htaNueva: Boolean(data.htaNueva ?? false),
      // Grados de Mallampati
      mallampatiGradoI: Boolean(data.mallampatiGradoI ?? false),
      mallampatiGradoII: Boolean(data.mallampatiGradoII ?? false),
      mallampatiGradoIII: Boolean(data.mallampatiGradoIII ?? false),
      mallampatiGradoIV: Boolean(data.mallampatiGradoIV ?? false),
      // Datos de conclusión de evaluación
      riesgoAlto: {
        criterioA: {
          si: Boolean(data.riesgoAltoCriterioASi ?? false),
          no: Boolean(data.riesgoAltoCriterioANo ?? false)
        },
        criterioB: {
          si: Boolean(data.riesgoAltoCriterioBSi ?? false),
          no: Boolean(data.riesgoAltoCriterioBNo ?? false)
        }
      },
      riesgoMedio: {
        criterioC: {
          si: Boolean(data.riesgoMedioCriterioCSi ?? false),
          no: Boolean(data.riesgoMedioCriterioCNo ?? false)
        },
        criterioD: {
          si: Boolean(data.riesgoMedioCriterioDSi ?? false),
          no: Boolean(data.riesgoMedioCriterioDNo ?? false),
          subcriterios: [
            { si: Boolean(data.riesgoMedioCriterioDSubcriterio0Si ?? false), no: Boolean(data.riesgoMedioCriterioDSubcriterio0No ?? false) },
            { si: Boolean(data.riesgoMedioCriterioDSubcriterio1Si ?? false), no: Boolean(data.riesgoMedioCriterioDSubcriterio1No ?? false) },
            { si: Boolean(data.riesgoMedioCriterioDSubcriterio2Si ?? false), no: Boolean(data.riesgoMedioCriterioDSubcriterio2No ?? false) },
            { si: Boolean(data.riesgoMedioCriterioDSubcriterio3Si ?? false), no: Boolean(data.riesgoMedioCriterioDSubcriterio3No ?? false) },
            { si: Boolean(data.riesgoMedioCriterioDSubcriterio4Si ?? false), no: Boolean(data.riesgoMedioCriterioDSubcriterio4No ?? false) },
            { si: Boolean(data.riesgoMedioCriterioDSubcriterio5Si ?? false), no: Boolean(data.riesgoMedioCriterioDSubcriterio5No ?? false) }
          ]
        },
        criterioE: {
          si: Boolean(data.riesgoMedioCriterioESi ?? false),
          no: Boolean(data.riesgoMedioCriterioENo ?? false)
        }
      },
      riesgoBajo: {
        si: Boolean(data.riesgoBajoSi ?? false),
        no: Boolean(data.riesgoBajoNo ?? false)
      },
      // Observaciones/Recomendaciones
    ...
    const drawHeader = (pageNumber) => {
      ...
      if (pageNumber === 1) {
        ...
        doc.text("FICHA DE DETECCIÓN DE S.A.S", pageW / 2, 32, { align: "center" });

        // Subtítulo
        doc.setFont("helvetica", "normal").setFontSize(10);
        doc.text("(Síndrome de Ápnea de sueño)", pageW / 2, 38, { align: "center" });

        // Tercera línea
        doc.setFont("helvetica", "normal").setFontSize(8);
        doc.text("(Conductores de maquinaria pesada, transporte de personal, transporte de materiales peligrosos)", pageW / 2, 42, { align: "center" });
      }
    ...
    doc.text(datosFinales.presionSistolica, tablaExamenInicioX + 20, yTextoExamen + 1);
    ...
    const imgWidth = 150; // Ancho de la imagen
    ...
    try {
      doc.addImage('./public/img/FichasSAS/epiglotis_pag2_normal.png', 'PNG', x, y, imgWidth, imgHeight);
    } catch (error) {
    ...
    // Dibujar fondo verde
    doc.setFillColor(0, 255, 0); // Verde #00ff00
...
B_FichaDetencionSAS2.propTypes = {
