...
export default async function InformePsicoBrigadista(data = {}, docExistente = null) {
  ...
  function buildDatosFinales(raw) {
    const datosFinales = {
      ...
      // Datos específicos para brigadistas - usando solo las variables de la data
      afrontamientoTomaDecisiones: String(raw?.afronTdd ?? ''),
      estiloConflicto: String(raw?.estiloConflicto ?? ''),
      afrontamientoSituacionesRiesgo: String(raw?.afronSitRiesgo ?? ''),
      nivelAnsiedad: String(raw?.levelAnsiedad ?? ''),
  ...
  const drawHeader = async (pageNumber) => {
    ...
    doc.text("INFORME PSICOLÓGICO / BRIGADISTAS", pageW / 2, 35, { align: "center" });
  ...
  const colCriterio = 100; // Ancho suficiente para "AFRONTAMIENTO A SITUACIONES DE RIESGO"

  // Definir criterios - 4 items
  const criterios = [
    { nombre: "AFRONTAMIENTO PARA LA TOMA DE DECISIONES", valor: datosFinales.afrontamientoTomaDecisiones || "" },
    { nombre: "ESTILO DE CONFLICTO", valor: datosFinales.estiloConflicto || "" },
    { nombre: "AFRONTAMIENTO A SITUACIONES DE RIESGO", valor: datosFinales.afrontamientoSituacionesRiesgo || "" },
    { nombre: "NIVEL DE ANSIEDAD", valor: datosFinales.nivelAnsiedad || "" }
  ];
  ...
  // Fila: FORTALEZAS / OPORTUNIDADES (con padding vertical)
  const anchoLabel = 58;
  ...
  // Fila: AMENAZAS / DEBILIDADES (con padding vertical)
