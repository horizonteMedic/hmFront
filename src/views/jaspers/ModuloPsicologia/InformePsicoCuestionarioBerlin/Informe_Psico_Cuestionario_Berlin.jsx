...
export default async function InformePsicoCuestionarioBerlin(data = {}, docExistente = null) {
  ...
  function buildDatosFinales(raw) {
    const datosFinales = {
      ...
      // Datos específicos para cuestionario de berlin - usando solo las variables de la data
      apneaObstructivaSueno: String(raw?.criterioApneaObstructivaSueno ?? ''),
      fatigaSomnolencia: String(raw?.criterioFatigaSomnolencia ?? ''),
      hipertensionArterial: String(raw?.criterioHipertensionArterial ?? ''),
  ...
  const drawHeader = async (pageNumber) => {
    ...
    doc.text("INFORME PSICOLÓGICO / CUESTIONARIO DE BERLIN", pageW / 2, 35, { align: "center" });
  ...
  const colCriterio = 100; // Ancho suficiente para "HIPERTENSIÓN ARTERIAL"

  // Definir criterios
  const criterios = [
    { nombre: "APNEA OBSTRUCTIVA DEL SUEÑO", valor: datosFinales.apneaObstructivaSueno || "" },
    { nombre: "FATIGA Y SOMNOLENCIA", valor: datosFinales.fatigaSomnolencia || "" },
    { nombre: "HIPERTENSIÓN ARTERIAL", valor: datosFinales.hipertensionArterial || "" }
  ];
