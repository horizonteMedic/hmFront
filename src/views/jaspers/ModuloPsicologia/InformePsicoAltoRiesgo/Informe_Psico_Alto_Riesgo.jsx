...
export default function InformePsicoAltoRiesgo(data = {}, docExistente = null) {
  ...
  function buildDatosFinales(raw) {
    const datosFinales = {
      ...
      // Datos específicos para alto riesgo - usando solo las variables de la data
      temorRiesgoElectrico: String(raw?.critTemorRiesgoElectrico ?? ''),
      temorTareaAltura: String(raw?.critTemorAlturasIzaje ?? ''),
      temorEspaciosConfinados: String(raw?.critTemorEspacConfi ?? ''),
  ...
  const drawHeader = (pageNumber) => {
    ...
    doc.text("INFORME PSICOLÓGICO FACTORES DE ALTO RIESGO", pageW / 2, 35, { align: "center" });
  ...
  const colCriterio = 100; // Ancho suficiente para "TEMOR A ESPACIOS CONFINADOS"

  // Definir criterios
  const criterios = [
    { nombre: "TEMOR A RIESGO ELÉCTRICO", valor: datosFinales.temorRiesgoElectrico || "" },
    { nombre: "TEMOR A TAREAS EN ALTURA", valor: datosFinales.temorTareaAltura || "" },
