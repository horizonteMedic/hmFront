...
export default async function InformePsicoExamComplementario(data = {}, docExistente = null) {
  ...
  const drawHeader = async (pageNumber) => {
    ...
    doc.text("INFORME PSICOLÓGICO/ EXAMENES COMPLEMENTARIOS", pageW / 2, 35, { align: "center" });
  ...
  // Tabla de criterios de evaluación - 6 ITEMS
  // Ancho fijo para la columna de criterios (suficiente para el más largo)
  const colCriterio = 100; // Ancho suficiente para "NIVEL DE COMPETENCIA SOCIAL"

  // Definir criterios - 6 items
  const criterios = [
    { nombre: "NIVEL DE AUTOCONCIENCIA", valor: datosFinales.nivelAutoconciencia || "" },
    { nombre: "NIVEL DE AUTOCONFIANZA", valor: datosFinales.nivelAutoconfianza || "" },
    { nombre: "NIVEL DE AUTORREGULACIÓN", valor: datosFinales.nivelAutorregulacion || "" },
    { nombre: "NIVEL DE MOTIVACIÓN", valor: datosFinales.nivelMotivacion || "" },
    { nombre: "NIVEL DE EMPATÍA", valor: datosFinales.nivelEmpatia || "" },
    { nombre: "NIVEL DE COMPETENCIA SOCIAL", valor: datosFinales.nivelCompetenciaSocial || "" }
  ];
