...
export default async function InformePsicoEspecificos(data = {}, docExistente = null) {
  ...
  function buildDatosFinales(raw) {
    const datosFinales = {
      ...
      // Datos específicos para trabajos específicos - usando solo las variables de la data
      nivelAlertaRiesgo: String(raw?.criterioNivelAlertRiesgo ?? ''),
      nivelPsicomotricidad: String(raw?.criterioNivelPsicomotrocidad ?? ''),
  ...
  const drawHeader = (pageNumber) => {
    ...
    doc.text("INFORME PSICOLÓGICO / TRABAJOS EN ESPECIFICO", pageW / 2, 35, { align: "center" });
  ...
  // Tabla de criterios de evaluación - SOLO 2 ITEMS
  // Ancho fijo para la columna de criterios (suficiente para el más largo)
  const colCriterio = 100; // Ancho suficiente para "NIVEL DE PSICOMOTRICIDAD"

  // Definir criterios - SOLO 2
  const criterios = [
    { nombre: "NIVEL DE ALERTA ANTE EL RIESGO", valor: datosFinales.nivelAlertaRiesgo || "" },
    { nombre: "NIVEL DE PSICOMOTRICIDAD", valor: datosFinales.nivelPsicomotricidad || "" }
  ];
