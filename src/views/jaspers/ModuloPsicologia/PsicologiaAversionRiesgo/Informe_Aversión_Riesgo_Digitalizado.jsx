...
export default async function Informe_Aversion_Riesgo_Digitalizado(data = {}, docExistente = null) {
  ...
  const convertirNivel = (alto, medio, bajo) => {
    if (alto) return "Alto";
  ...
  const datos = {
    ...
    practicaFuncional: convertirNivel(
      data.aspIntelPractFuncAlto,
      data.aspIntelPractFuncMedio,
      data.aspIntelPractFuncBajo
    ),
    recursividad: convertirNivel(
      data.aspIntelRecurAlto,
      data.aspIntelRecurMedio,
      data.aspIntelRecurBajo
    ),
    capacidadAtencionConcentracion: convertirNivel(
      data.aspIntelAtenciConcAlto,
      data.aspIntelAtenciConcMedio,
      data.aspIntelAtenciConcBajo
    ),
    // Aspectos Emocionales
    estabilidadEmocional: convertirNivel(
      data.aspEmocEstabilEmocMadAlto,
      data.aspEmocEstabilEmocMadMedio,
      data.aspEmocEstabilEmocMadBajo
    ),
    flexibilidadManejoEmociones: convertirNivel(
      data.aspFlexibManjEmocAlto,
      data.aspFlexibManjEmocMedio,
      data.aspFlexibManjEmocBajo
    ),
    controlImpulsos: convertirNivel(
      data.aspCtrlImpulAlto,
      data.aspCtrlImpulMedio,
      data.aspCtrlImpulBajo
    ),
    // Competencias Específicas de Conducta Segura
    capacidadSubordinacion: convertirNivel(
      data.compEspecfCapSuborAlto,
      data.compEspecfCapSuborMedio,
      data.compEspecfCapSuborBajo
    ),
    adecuacionNormas: convertirNivel(
      data.compEspecfAdecNorProcedAlto,
      data.compEspecfAdecNorProcedMedio,
      data.compEspecfAdecNorProcedBajo
    ),
    consideracionTerceros: convertirNivel(
      data.compEspecfConsideraTercerosAlto,
      data.compEspecfConsideraTercerosMedio,
      data.compEspecfConsideraTercerosBajo
    ),
    autonomiaTrabajo: convertirNivel(
      data.compEspecfAutonomiaTrabajarAlto,
      data.compEspecfAutonomiaTrabajarMedio,
      data.compEspecfAutonomiaTrabajarBajo
    ),
    proactividad: convertirNivel(
      data.compEspecfProactividadAlto,
      data.compEspecfProactividadMedio,
      data.compEspecfProactividadBajo
    ),
    capacidadTrabajoPresion: convertirNivel(
      data.compEspecfCapTrabjoBajoPresionAlto,
      data.compEspecfCapTrabjoBajoPresionMedio,
      data.compEspecfCapTrabjoBajoPresionBajo
    ),
    capacidadEvaluarRiesgos: convertirNivel(
      data.compEspecfCapEvaluarRiesgosAlto,
      data.compEspecfCapEvaluarRiesgosMedio,
      data.compEspecfCapEvaluarRiesgosBajo
    ),
    motivacionCargo: convertirNivel(
      data.compEspecfMotPorCarAlto,
      data.compEspecfMotPorCarMedio,
      data.compEspecfMotPorCarBajo
    ),
    // Análisis y Resultados
    analisisResultados: String(data.analisisResultados ?? ""),
    recomendaciones: String(data.recomendaciones ?? ""),
    // Conclusiones
    cumplePerfil: data.conclusionesCumple ?? false,
  ...
  doc.text("INFORME DE PSICOLÓGICA DE AVERSIÓN AL RIESGO", pageW / 2, 35, { align: "center" });
  ...
  yPos = dibujarFilaAspecto("Aspectos\nIntelectuales", "Practica y Funcional", datos.practicaFuncional, yPos, 3, true);
  yPos = dibujarFilaAspecto("", "Recursividad", datos.recursividad, yPos, 0, false);
  yPos = dibujarFilaAspecto("", "Capacidad de atención y concentración", datos.capacidadAtencionConcentracion, yPos, 0, false);

  // Aspectos Emocionales (3 filas)
  yPos = dibujarFilaAspecto("Aspectos\nEmocionales", "Estabilidad emocional - madurez", datos.estabilidadEmocional, yPos, 3, true);
  yPos = dibujarFilaAspecto("", "Flexibilidad en el manejo de las emociones", datos.flexibilidadManejoEmociones, yPos, 0, false);
  yPos = dibujarFilaAspecto("", "Control de impulsos", datos.controlImpulsos, yPos, 0, false);

  // Competencias Específicas de Conducta Segura (8 filas)
  yPos = dibujarFilaAspecto("Competencias\nEspecíficas de\nConducta Segura", "Capacidad de subordinación", datos.capacidadSubordinacion, yPos, 8, true);
  yPos = dibujarFilaAspecto("", "Adecuación a las normas y procedimientos", datos.adecuacionNormas, yPos, 0, false);
  yPos = dibujarFilaAspecto("", "Consideración de terceros", datos.consideracionTerceros, yPos, 0, false);
  yPos = dibujarFilaAspecto("", "Autonomía para trabajar", datos.autonomiaTrabajo, yPos, 0, false);
  yPos = dibujarFilaAspecto("", "Proactividad", datos.proactividad, yPos, 0, false);
  yPos = dibujarFilaAspecto("", "Capacidad para trabajar bajo presión", datos.capacidadTrabajoPresion, yPos, 0, false);
  yPos = dibujarFilaAspecto("", "Capacidad para evaluar riesgos", datos.capacidadEvaluarRiesgos, yPos, 0, false);
  yPos = dibujarFilaAspecto("", "Motivación por el cargo", datos.motivacionCargo, yPos, 0, false);
  ...
  if (datos.analisisResultados && doc.getTextWidth(datos.analisisResultados) > tablaAncho - 4) {
  ...
  if (datos.analisisResultados) {
    if (doc.getTextWidth(datos.analisisResultados) > tablaAncho - 4) {
      const lineas = doc.splitTextToSize(datos.analisisResultados, tablaAncho - 4);
      lineas.forEach((linea, idx) => {
        doc.text(linea, tablaInicioX + 2, yPos + padding + 2 + (idx * 3.5));
      });
    } else {
      doc.text(datos.analisisResultados, tablaInicioX + 2, yPos + padding + 2);
    }
  }
  yPos += alturaAnalisis;
  ...
  if (datos.recomendaciones && doc.getTextWidth(datos.recomendaciones) > tablaAncho - 4) {
  ...
  // Restablecer fuente a normal para el contenido
  doc.setFont("helvetica", "normal").setFontSize(8);
  if (datos.recomendaciones) {
    if (doc.getTextWidth(datos.recomendaciones) > tablaAncho - 4) {
      const lineas = doc.splitTextToSize(datos.recomendaciones, tablaAncho - 4);
      lineas.forEach((linea, idx) => {
        doc.text(linea, tablaInicioX + 2, yPos + padding + 2 + (idx * 3.5));
      });
    } else {
      doc.text(datos.recomendaciones, tablaInicioX + 2, yPos + padding + 2);
    }
  }
  ...
  let firmaPsicologoUrl = getSign(datos, "SELLOFIRMA");
  ...
  const cumplePerfil = datos.cumplePerfil ?? true;
