...
export default async function InformePsicoCuadradorVigia(data = {}, docExistente = null) {
  ...
  function buildDatosFinales(raw) {
    const datosFinales = {
      ...
      fechaNacimiento: formatearFechaCorta(raw?.fechaNacimiento ?? raw?.fechaNacimientoPaciente ?? ''),
      domicilio: String(raw?.direccionPaciente ?? raw?.direccion ?? ''),
      areaTrabajo: String(raw?.area ?? raw?.areaPaciente ?? raw?.ocupacion ?? raw?.ocupacionPaciente ?? ''),
      puestoTrabajo: String(raw?.cargo ?? raw?.cargoPaciente ?? raw?.cargoDesempenar ?? ''),
      ...
      // Datos específicos para cuadrador vigía - mapeo directo desde la data del controller
      temorRiesgoElectrico: String(raw?.temorRiesgoElectrico ?? raw?.criterioTemorRiesgoElectrico ?? ''),
      temorTareaAltura: String(raw?.temorTareasAltura ?? raw?.temorTareaAltura ?? raw?.criterioTemorTareaAltura ?? ''),
      temorEspaciosConfinados: String(raw?.temorEspaciosConfinados ?? raw?.criterioTemorEspaciosConfinados ?? ''),
      manejoHerramientas: String(raw?.manejoHerramientas ?? raw?.manejoDeHerramientas ?? raw?.criterioManejoHerramientas ?? ''),
      fortalezasOportunidades: String(raw?.fodaForOpor ?? raw?.fortalezasOportunidades ?? raw?.analisisFodaFortalezasOportunidades ?? raw?.analisisFodaFortaOport ?? ''),
      amenazasDebilidades: String(raw?.fodaAmenDebi ?? raw?.amenazasDebilidades ?? raw?.analisisFodaAmenazasDebilidades ?? raw?.analisisFodaAmenazDebili ?? ''),
      observaciones: String(raw?.observacion ?? raw?.observaciones ?? ''),
      recomendaciones: String(raw?.recomenda ?? raw?.recomendaciones ?? raw?.recomendacion ?? ''),
      cumplePerfil: (typeof raw?.cumplePerfil === 'boolean') ? raw.cumplePerfil : (raw?.cumplePerfil === true || raw?.cumplePerfil === 'true' || raw?.cumplePerfil === 1 || raw?.perfilCumple === true || raw?.perfilCumple === 'true' || raw?.perfilCumple === 1),
    };
    return datosFinales;
  }
  ...
  const drawHeader = async (pageNumber) => {
    ...
    doc.text("INFORME PSICOLÓGICO PARA CUADRADOR VIGIA", pageW / 2, 35, { align: "center" });
  ...
  // Dibujar filas de criterios con altura dinámica
  let yFilaActual = yPos;
  criterios.forEach((criterio) => {
    ...
    // Texto del criterio (centrado verticalmente)
    doc.setFont("helvetica", "normal").setFontSize(9);
    const centroVerticalCriterio = yFilaActual + alturaFila / 2;
    doc.text(criterio.nombre, tablaInicioX + 2, centroVerticalCriterio);

    // Texto del resultado con salto de línea
    doc.setFont("helvetica", "normal").setFontSize(9);
    if (lineasValor.length === 1) {
      // Una sola línea, centrar verticalmente
      doc.text(lineasValor[0], tablaInicioX + colCriterio + 2, centroVerticalCriterio);
    } else {
      // Múltiples líneas, empezar desde arriba con padding
      const yInicioTexto = yFilaActual + 2;
  ...
  // Actualizar yPos con la posición final
  yPos = yFilaActual;
  ...
  // Fila: FORTALEZAS / OPORTUNIDADES (con padding vertical y altura dinámica)
  const anchoLabel = 58;
  ...
  // Fila: AMENAZAS / DEBILIDADES (con padding vertical y altura dinámica)
  ...
  doc.setFont("helvetica", "bold").setFontSize(9);
  const centroVertical = yPos + filaAltura / 2 + 1;
  ...
  // Ajustar centro vertical para la X (fontSize 12) para que quede mejor centrada
  const cumplePerfil = datosFinales.cumplePerfil ?? true;
  doc.setFont("helvetica", "bold").setFontSize(12);
  const centroVerticalX = yPos + filaAltura / 2 + 1.5; // Ajustado para centrar mejor la X más grande
  if (cumplePerfil) {
    // Marcar X en la columna vacía después de SI (columna 2)
    doc.text("X", tablaInicioX + colTextoW + colVaciaW / 2, centroVerticalX, { align: "center" });
  } else {
    // Marcar X en la columna vacía después de NO (columna 4)
    doc.text("X", tablaInicioX + colTextoW * 2 + colVaciaW + colVaciaW / 2, centroVerticalX, { align: "center" });
  }
