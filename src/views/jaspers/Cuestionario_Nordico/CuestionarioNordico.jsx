...
import { compressImage, getSign, getSignCompressed } from "../../utils/helpers";
...
export default async function CuestionarioNordico(datos = {}, docExistente = null) {
  ...
  // 2) Encabezado (logo, campos, título)
  const sellofirma = await getSign(datos, "SELLOFIRMA");
  const firmap = await getSign(datos, "FIRMAP");
  const huellap = await getSign(datos, "HUELLA");

  // Continuamos con el flujo normal (estructura aplanada
  // Alias para compatibilidad temporal o uso directo de las variables nuevas
  // 2) Encabezado (logo, campos, título)
  // 2) Encabezado (logo, campos, título)
  ...
  autoTable(doc, {
    ...
    didDrawCell: (data) => {
      ...
      if (data.row.index === 0 && data.column.index === 0) {
        ...
        // ===== Agregar firmap (izquierda del medio) =====
        if (firmap) {
          const centerX = (lineStart + lineMid) / 2; // punto medio de la izquierda
          const imgX = centerX - maxImgW / 2;
          const imgY = textY + 10 - maxImgH - 2;
          doc.addImage(firmap, 'JPEG', imgX, imgY, maxImgW, maxImgH);
        }

        // ===== Agregar huellap (derecha del medio) =====
        if (huellap) {
          const centerX = (lineMid + lineEnd) / 2; // punto medio de la derecha
          const imgX = centerX - maxImgW / 2;
          const imgY = textY + 10 - maxImgH - 2;
          doc.addImage(huellap, 'JPEG', imgX, imgY, maxImgW, maxImgH);
        }
