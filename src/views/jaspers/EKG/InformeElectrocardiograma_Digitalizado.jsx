...
export default async function InformeElectrocardiograma_Digitalizado(data = {}, docExistente = null) {
  const doc = docExistente || new jsPDF({ unit: "mm", format: "a4", orientation: "portrait" });
  const pageW = doc.internal.pageSize.getWidth();
  console.log("jaseper digitalizado");
  ...
  doc.setFont("helvetica", "normal").setFontSize(8);
  doc.text(dnicompletarConCeros(datosFinales.dni), tablaInicioX + 12, textoY2);
  ...
  const dibujarFilaParametroEKG = (label, valor) => {
    ...
    // Contenido de la fila - centrado verticalmente
    const textoY = yPos + (filaAltura / 2) + (8 * 0.35) / 2;
    doc.setFont("helvetica", "bold").setFontSize(8);
    doc.text(label + ":", tablaInicioX + 2, textoY);
    doc.setFont("helvetica", "normal").setFontSize(8);
    doc.text(valor || "", tablaInicioX + 65, textoY);
