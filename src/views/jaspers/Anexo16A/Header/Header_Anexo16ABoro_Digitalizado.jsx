const headerAnexo16ABoro_Digitalizado = (doc, datos, numeroPagina = 1) => {
  const pageW = doc.internal.pageSize.getWidth();

  // Márgenes de 8mm a cada lado (consistente con Anexo16)
  ...
  // Datos fijos para Anexo16A Boro Digitalizado
  const datosFijos = {
    ...
    codigoColor: "#008f39",
    textoColor: "F",
    nombresApellidos: "CASTILLO PLASENCIA HADY KATHERINE",
    fechaExamen: "15/12/2024",
    sexo: "F",
    dni: "12345678",
    edad: "28",
    areaTrabajo: "MINERÍA",
    puestoTrabajo: "OPERADOR DE EQUIPOS",
  ...
  const datosReales = {
    ...
    nombresApellidos: datos?.nombres_nombres ?? "",
    ...
    tipoExamen: datos?.nombreExamen_nom_examen ?? "ANEXO 16-A BORO",
  };

  const datosFinales = datos && Object.keys(datos).length > 0 ? datosReales : datosFijos;
  ...
  // 4) SECCIONES DE DATOS FIJOS DEL PACIENTE
  ...
  // === FILA 1: NOMBRES Y APELLIDOS + FECHA EXAMEN ===
  // NOMBRES Y APELLIDOS (columna izquierda)
  doc.setFont("helvetica", "bold").setFontSize(9);
  doc.text("NOMBRES Y APELLIDOS:", col1X, dataY);
  doc.setFont("helvetica", "normal").setFontSize(8);

  // Calcular ancho disponible para el texto de nombre
  const nombreLabelWidth = doc.getTextWidth("NOMBRES Y APELLIDOS:");
  ...
  const nombreLines = doc.splitTextToSize(datosFinales.nombresApellidos, nombreMaxWidth);
  let nombreY = dataY;
  ...
  // FECHA EXAMEN (columna derecha)
  doc.setFont("helvetica", "bold").setFontSize(9);
  doc.text("FECHA EXAMEN:", col2X, dataY);
  doc.setFont("helvetica", "normal").setFontSize(8);
  doc.text(datosFinales.fechaExamen, col2X + 30, dataY);

  // Ajustar dataY basado en la fila más larga
  const fila1MaxY = Math.max(nombreY, dataY);
  dataY = fila1MaxY + 4;

  // === FILA 2: SEXO + DNI + EDAD ===
  // SEXO (columna izquierda)
  doc.setFont("helvetica", "bold").setFontSize(9);
  doc.text("SEXO:", col1X, dataY);
  doc.setFont("helvetica", "normal").setFontSize(8);
  doc.text(datosFinales.sexo, col1X + 15, dataY);

  // DNI (columna izquierda, después de SEXO)
  doc.setFont("helvetica", "bold").setFontSize(9);
  doc.text("DNI:", col1X + 50, dataY);
  doc.setFont("helvetica", "normal").setFontSize(8);
  doc.text(datosFinales.dni, col1X + 70, dataY);

  // EDAD (columna derecha)
  doc.setFont("helvetica", "bold").setFontSize(9);
  doc.text("EDAD:", col2X, dataY);
  doc.setFont("helvetica", "normal").setFontSize(8);
  doc.text(datosFinales.edad, col2X + 20, dataY);

  dataY += 4;

  // === FILA 3: AREA DE TRABAJO + PUESTO DE TRABAJO ===
  // AREA DE TRABAJO (columna izquierda)
  doc.setFont("helvetica", "bold").setFontSize(9);
  doc.text("AREA DE TRABAJO:", col1X, dataY);
  doc.setFont("helvetica", "normal").setFontSize(8);
  doc.text(datosFinales.areaTrabajo, col1X + 40, dataY);

  // PUESTO DE TRABAJO (columna derecha)
  doc.setFont("helvetica", "bold").setFontSize(9);
  doc.text("PUESTO DE TRABAJO:", col2X, dataY);
  doc.setFont("helvetica", "normal").setFontSize(8);
  doc.text(datosFinales.puestoTrabajo, col2X + 40, dataY);

  dataY += 4;

  // === FILA 4: EMPRESA (ancho completo) ===
  doc.setFont("helvetica", "bold").setFontSize(9);
  ...
  // === FILA 5: CONTRATA (ancho completo) ===
  doc.setFont("helvetica", "bold").setFontSize(9);
...
export default headerAnexo16ABoro_Digitalizado;
