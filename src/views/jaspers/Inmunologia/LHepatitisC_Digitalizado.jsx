...
export default async function LHepatitisC_Digitalizado(datos = {}, docExistente = null) {
  ...
  await Promise.all([
    ...
    if (resultadoHepatitisC) {
      ...
      if (resultadoLower.includes('positivo') || resultadoLower === 'positivo' || resultadoLower === 'pos' || resultadoLower === '+') {
        resultadoTexto = 'Positivo';
      } else if (resultadoLower.includes('negativo') || resultadoLower === 'negativo' || resultadoLower === 'neg' || resultadoLower === '-') {
        resultadoTexto = 'Negativo';
      } else {
        resultadoTexto = resultadoHepatitisC; // Mantener el valor original si no coincide
      }
    }

    doc.text(resultadoTexto, 105 + 40, yTable, { align: 'left' });
