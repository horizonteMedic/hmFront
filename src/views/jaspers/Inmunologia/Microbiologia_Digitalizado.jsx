...
export default async function Microbiologia_Digitalizado(datos = {}, docExistente = null) {
  ...
  await Promise.all([
    ...
    if (datos.examenDirecto) {
      doc.text("EXAMEN DIRECTO (KOH)", config.col1X, y);
      doc.text(datos.txtKoh ?? "N/A", config.col2X, y, { align: "center" });
    } else {
      doc.text("Examen de BK - BACILOSCOPIA 1° Muestra", config.col1X, y);
      doc.text(datos.txtMuestra1 ?? "N/A", config.col2X, y, { align: "center" });
      y += config.lineHeight;
      doc.text("Examen de BK - BACILOSCOPIA 2° Muestra", config.col1X, y);
      doc.text(datos.txtMuestra2 ?? "N/A", config.col2X, y, { align: "center" });
    }
