...
export const VerifyTR = async (form, get, token, set, sede, setHTR, setH) => {
    if (!form.nro) {
    ...
    get(`/api/v01/ct/consentDigit/existenciaExamenes?nOrden=${form.nro}&nomService=${'triaje'}`, token)
        .then((res) => {
            if (res.id === 0) {
                ...
                GetListTriajeMult(form.nro, set, get, token, false, setHTR);
            }
        });
};
...
export const GetTable = (nro, nombre, sede, token, set) => {
    ...
    GetHistoriaCTriaje(data, sede, token).then((res) => {
        set(res);
    });
};

export const SearchHC = (event, form, set, sede, token) => {
    if (event.key === 'Enter') {
        const data = {
            opcion_id_p: 2,
            norden: Number(form.codigo),
            nombres_apellidos_p: '',
        };
        GetHistoriaCTriaje(data, sede, token).then((res) => {
            set(res);
...
export const handleNombreChange = (e, set, setTable, sede, token, debounceTimeout) => {
    ...
    set((prev) => ({ ...prev, nombresBusqueda: value, codigo: '' }));
            ...
            if (res.id === 1) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        GetListTriajeMult(nro, set, get, token, true);
                    }
                });
            } else if (res.id === 0) {
                refreshtable();
...
// FUNCIÓN MULTIPLE DE LOBO
export const GetListTriajeMult = async (nro, set, get, token, jasper, setHTR) => {
    if (!nro) {
        await Swal.fire('Error', 'Debe Introducir un Nro de Historia Clinica valido', 'error');
        return;
    }
        ...
        .then(async (res) => {
            if (res.n_orden) {
                ...
                set((prev) => ({
                    ...
                    fCardiaca: res.f_cardiaca,
                    sat02: res.sat_02,
                    perimetroCuello: res.perimetro_cuello,
...
export const GetListTriajeMulttable = async (nro, set, get, token, setHTR, setH) => {
    if (!nro) {
        await Swal.fire('Error', 'Debe Introducir un Nro de Historia Clinica valido', 'error');
        return;
    }
...
export const GetJasper = async (nro) => {
    ReporteTriaje(nro);
};
