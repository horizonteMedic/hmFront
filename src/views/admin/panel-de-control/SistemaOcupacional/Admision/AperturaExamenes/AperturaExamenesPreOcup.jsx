...
import HojadeRutaDinamico from '../../../../../jaspers/HojadeRutaDinamico/HojadeRutaDinamico.jsx';
...
const AperturaExamenesPreOcup = (props) => {
  ...
  //Traer listas de Examenes Adicionales
  useEffect(() => {
    if (datos.idProtocolo) {
      ...
      getFetch(`/api/v01/ct/protocolo/examenAdicional/obtenerExamenesAdicionales/${datos.idProtocolo}`, props.token)
        .then((res) => {
          if (res.estatus === "OK") {
  ...
  const handleLimpiar = e => {
    ...
    setOriginalExams([]); // Limpiar exámenes originales
    RendeSet("")
    props.ChangeDNI("")
    setShowEdit(false)
    setHabilitar(false)
  ...
  const SearchClickRight = (n_orden) => {
    ...
    getFetch(`/api/v01/ct/consentDigit/busquedaHistoriaOcupNOrden/${n_orden.n_orden}`, props.token)
      .then((res) => {
        InfoHR2(res.n_orden, res.nombres);
      })
  }

  const InfoHR2 = (HC, nombres) => {
        ...
        <div className="w-full sm:w-full md:w-auto text-lg font-semibold  ">
          ...
          <div className="flex items-center space-x-2 mb-1">
            ...
            <div className="flex flex-wrap pt-2 pb-2">
              ...
              {datos.examenesAdicionales && datos.examenesAdicionales.length > 0 && <div className='w-full flex flex-wrap bg-[#f1f5f980] rounded-lg p-3 gap-2' >
                {datos.examenesAdicionales?.map((examen) => (
                  <div className='py-2 px-3 flex items-center bg-slate-300 .bg-slate-300 border-2 rounded-xl font-semibold gap-3'>
                    ...
                    <button onClick={() => { removeExamen(examen.id ? examen.id : examen.idExamen) }}>X</button>
                  </div>
                ))}
