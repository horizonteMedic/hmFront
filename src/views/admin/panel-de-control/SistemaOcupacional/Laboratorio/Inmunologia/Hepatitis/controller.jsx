...
import { SubmitHepatitis } from "../model/model.js";
...
export const VerifyTR = async (nro,tabla,token,set,sede) => {
    ...
    .then((res) => {
        ...
        if (res.id === 0) {
            ...
            GetInfoHepatitisLab(nro,tabla,set,token)
        }
    })
}
...
export const GetInfoHepatitisLab = (nro,tabla,set,token) => {
...
export const SubmitHepatitisLab = async (form,token,user,limpiar,tabla) => {
    if (!form.norden) {
    ...
    console.log(token)
    SubmitHepatitis(form,user,token)
    .then((res) => {
        console.log(res)
