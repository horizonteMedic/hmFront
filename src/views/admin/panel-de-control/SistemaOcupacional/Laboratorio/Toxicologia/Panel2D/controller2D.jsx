...
import { SubmitToxPanel2D } from "../model/model.js";
...
export const GetInfoPanel2D = (nro,tabla,set,token) => {
  getFetch(`/api/v01/ct/toxicologia/obtenerReportePanel2D?nOrden=${nro}&nameService=${tabla}`,token)
  .then((res) => {
    if (res.norden) {
       Swal.fire(
          "Alerta",
      ...
      set(prev => ({
        ...
        valueM: res.reMarihuana,
        valueC: res.reCocaina,
...
export const SubmitPanel2D = async (form,user,token,limpiar,tabla) => {
  ...
  SubmitToxPanel2D(form,user, token)
  .then((res) => {
    console.log(res)
...
export const PrintHojaR = async (norden,tabla,token) => {
  ...
  getFetch(`/api/v01/ct/toxicologia/obtenerReportePanel2D?nOrden=${norden}&nameService=${tabla}`,token)
  .then(async (res) => {
    if (res.norden) {
