...
const Panel4D = ({token, selectedSede, userlogued}) => {
  const date = new Date();
  ...
  const handlePrint = () => {
    ...
    Swal.fire({
      title: '¿Desea Imprimir Consentimiento Panel 4D?',
      ...
      if (result.isConfirmed) {
        PrintHojaR(form,'con_panel4D',token);
      }
    });
  };
    ...
    <form className="w-full max-w-7xl mx-auto bg-white p-8 rounded shadow">
      <div className="flex flex-wrap items-center gap-6 mb-6">
        <div className="flex items-center gap-2">
          ...
          onKeyUp={(event) => {if(event.key === 'Enter')handleset(),VerifyTR(form.norden,'con_panel4D',token,setForm,selectedSede,form)}} />
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="text-center font-bold text-xl mb-4">
        CONSENTIMIENTO INFORMADO PARA REALIZAR LA PRUEBA DE DOSAJE DE COCAÍNA, MARIHUANA, OPIÁCEOS Y METHANFETAMINAS.
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 mt-6 items-center justify-between">
        <div className="flex gap-3">
          <button type="button" onClick={(() => {SubmitConsentimientoLab(form,"con_panel4D",token, userlogued, form.antecedentes.COCA ? form.fechaCoca : null, false, handleLimpiar)})} className="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-2 rounded flex items-center gap-2 font-semibold shadow-md transition-colors">
