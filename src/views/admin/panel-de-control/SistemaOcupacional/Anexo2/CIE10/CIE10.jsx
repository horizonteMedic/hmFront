...
const CIE10 = ({ closeModal, token, setForm }) => {
    ...
    const inserCIE10 = () => {
        if (selectedResult) {
            const nuevoDiagnostico = `CIE 10: ${selectedResult.codigo} - ${selectedResult.descripcion}`;

            setForm(prev => ({
                ...prev,
                observacionesGenerales: prev.observacionesGenerales
                    ? `${prev.observacionesGenerales}\n${nuevoDiagnostico.toUpperCase()}`
                    : nuevoDiagnostico
            }));
