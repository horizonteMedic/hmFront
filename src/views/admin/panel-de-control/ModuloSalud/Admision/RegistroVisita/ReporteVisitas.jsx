...
const urlSubmit = "/api/reportes/pacientes"

const ReporteVisitas = ({ onClose, sede, token }) => {
    ...
    const GetReporte = () => {
        ...
        const data = {
            ...
            "fechaHasta": form.fechaFin
        }
        getFetch(`${urlSubmit}?fechaDesde=${form.fechaInicio}&fechaHasta=${form.fechaFin}`, token)
            .then((res) => {
    ...
    const generarExcelReporte = async (data, fechaInicio, fechaFin) => {
        ...
        titleCell.value = `REPORTE DE REGISTRO GENERAL DE ASISTENCIA | ${fechaInicio} al ${fechaFin}`;
        ...
        const headers = [
            ...
            { key: "fechaRegistro", label: "Fecha", width: 14, destacado: false },
            { key: "apellidosNombres", label: "Nombres y Apellidos", width: 40, destacado: true },
            { key: "dni", label: "DNI", width: 14, destacado: true },
            { key: "edad", label: "EDAD", width: 8, destacado: false },
            { key: "fechaNacimiento", label: "Fecha de Nacimiento", width: 24, destacado: false },
            { key: "tallaCm", label: "Talla", width: 18, destacado: false },
            { key: "pesoKg", label: "Peso", width: 16, destacado: false },
            { key: "apellidosNombresPadreMadre", label: "APELLIDOS Y NOMBRES DEL PADRE O MADRE", width: 50, destacado: false },
            { key: "dniPadreMadre", label: "DNI Padre o Madre", width: 14, destacado: false },
            { key: "firma", label: "Firma", width: 15, destacado: false },
            { key: "especialidad", label: "Especialidad", width: 50, destacado: false },
        ];
        ...
        saveAs(new Blob([buffer]), `Reporte_Visitas_${fechaInicio}_${fechaFin}.xlsx`);
    };
