...
import { editContrata } from '../../model/AdministrarContrata';
const EditModal = ({ setShowEditModal,Refresgpag,ruc,token, razon,direccion,telefono,responsable, email }) => {
  ...
  function AleertSucces() {
    Swal.fire({
      title: "¡Exito!",
      text: "Se ha editado la Contrata",
      icon: "success",
  ...
  const handleSaveContrata = () => {
    setCreating(true)
    editContrata  (ruc,arazonSocial,adireccion,atelefono,aresponsable,aemail,token)
        .then(data => {
          AleertSucces()
        })
        .catch(error => {
    ...
    <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="mx-auto bg-white rounded-lg overflow-hidden shadow-md w-[400px] relative">
        ...
        <div className="p azuloscurobackground flex justify-between p-3.5">
          <h1 className="text-start font-bold color-azul text-white">Editar Contrata</h1>
        </div>
